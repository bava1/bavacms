...
class edit_stati extends Adminer {
	...
	public function get_Content() {
		...
		Add a new article :</a></div>";
		echo "<div class='cat2' style='font-size: 24px'>List of articles available for modification:</div>";
		$query = "SELECT id, title FROM stati";
		$result = mysqli_query($this->db, $query);
		if (!$result) {
			exit('Could not get to admin articles');
		}
		...
		for ($i=0; $i < mysqli_num_rows($result); $i++) {
			...
			printf("<div class='cat2'><p style='font-size: 18px'><a style='color: green' 
				...
				<a class='cat4' href='?options=delete_stati&id_text=%s'> | Delete article</a>
